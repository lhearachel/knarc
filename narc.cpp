...
// a lot of the code in this function is duplicated in anticipation of deprecating the existing input API
// in favor of subcommands
extern narc::NarcError narc::pack(const fs::path &dst_file, const std::vector<std::string> &src_files)
{
    ...
    for (auto &src_file : src_files) {
        fs::directory_entry entry(src_file);
        if (!entry.exists()) {
            cerr << "FATAL: File " << src_file << " does not exist" << endl;
